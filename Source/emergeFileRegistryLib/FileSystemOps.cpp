  ...
  while (nextSeparator != std::wstring::npos)
    {
      ...
      if ((ELExhaustivelyFindFilePath(pathTmp) != TEXT("")) || (IsInternalCommand(pathTmp)))
        {
...
std::wstring ELStripFileArgs(std::wstring filePath)
{
...
std::wstring ELGetRelativePath(std::wstring filePath, std::wstring baseDirPath)
{
  ...
  unc = ELGetUNCFromMap(program, MAX_PATH);
  if (!unc.empty())
    wcscpy(program, unc.c_str());
  unc = ELGetUNCFromMap(fullyExpandedBasePath.c_str(), MAX_PATH);
  if (!unc.empty())
...
bool ELExecuteFileOrCommand(std::wstring application, std::wstring arguments, std::wstring workingDir, int nShow)
{
  ...
  {
    result = ExecuteInternalCommand(application, arguments);
  }

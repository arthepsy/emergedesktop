...
INT_PTR Config::DoInitDialog(HWND hwndDlg)
{
  ...
  psp[1].pszTemplate = MAKEINTRESOURCE(IDD_STICKY_PAGE);
  psp[1].pfnDlgProc = pStickyPage->StickyPageDlgProc;
  psp[1].pszTitle = TEXT("Sticky Items");
  psp[1].lParam = reinterpret_cast<LPARAM>(pStickyPage.get());
  ...
  psh.pszCaption = TEXT("emergeVWM Configuration");
